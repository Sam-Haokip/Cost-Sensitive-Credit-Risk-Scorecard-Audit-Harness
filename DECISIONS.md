...
## 13. Exact SHAP for the linear model instead of a general-purpose library call, a hand-rolled dtype-safe permutation shuffle instead of scikit-learn's, and a real disagreement between the two on `addr_state`

**Decision.** Both models are explained in log-odds-of-default, the unit both already compute in: LightGBM's `predict(..., raw_score=True)` is a log-odds margin, and the logistic-WoE scorecard's linear predictor is log-odds by construction (that is the reason WoE encoding exists — see `features/woe.py`). For the logistic model, SHAP values are computed with a hand-derived closed form, `coef_i * (x_i - E[x_i])`, rather than a call to `shap.LinearExplainer`, because a linear model's Shapley value has *no approximation to make*: it is additively separable, so there are no interaction terms to apportion credit for. For LightGBM, which has no such closed form, `shap.TreeExplainer` computes the exact (not sampled) Shapley values via TreeSHAP. Permutation importance uses a hand-rolled loop (`permutation_importance_manual`) rather than `sklearn.inspection.permutation_importance`.

**Why not just call the library and trust it — two things checked, not assumed.** `shap.LinearExplainer(model, background_df)` was tried first, and its output disagreed with the closed form by up to 0.044 in log-odds units on this project's real fitted model — enough to change a feature's rank. Root cause, found by inspecting the library rather than guessing: its default `Independent` masker silently subsamples the background data to 100 rows regardless of how much is actually passed in (`shap.maskers.Independent(data)` defaults to `max_samples=100`). Forcing `max_samples=len(data)` reproduces the closed form to floating-point precision (`tests/test_explain.py::test_linear_shap_values_matches_shap_linear_explainer_with_full_background`); a companion test confirms the library really does disagree with the *default* setting on the same data (`test_linear_shap_values_disagrees_with_shap_default_100_row_subsample`), so this isn't reported as a bug from memory — it's reproduced in the test suite. Separately, `sklearn.inspection.permutation_importance` hung for multiple minutes on this project's real categorical-feature data at settings that should take under a minute; not fully root-caused, but a from-scratch permutation attempt surfaced a concrete, fixable bug in the *naive* approach it was compared against: `rng.permutation(X[col].values)` on a pandas `category`-dtype column silently strips the dtype (`numpy.random.Generator.permutation` calls `np.asarray` internally), and LightGBM's `predict` then raises `"train and valid dataset categorical_feature do not match"`. Fixed by shuffling via positional reindexing (`X[col].iloc[perm]`), which preserves dtype exactly — regression-tested directly (`test_shuffle_column_preserves_categorical_dtype`, `test_shuffle_column_naive_numpy_permutation_would_strip_the_dtype`).

**Measured cost, not assumed to be symmetric across models.** Permutation importance is run on a fixed 10,000-row subsample of each test fold (96 features × 5 repeats), timed per model rather than assumed equal: ~200s/fold for logistic_woe, ~63s/fold for lightgbm — the gap is `WOEEncoder.transform` itself, called fresh inside every one of the 480 shuffled predict calls (~0.31s each on 10,000 × 96). SHAP's closed-form linear computation is negligible; `TreeExplainer` on the full 100,000-row test fold is not, at ~130s/fold, the single largest piece of one fold's runtime. Total measured: ~28 minutes for all 4 folds plus a one-time ~50s data load — stated in the module docstring so the number is available before anyone runs it, not discovered by waiting.

**Headline finding: SHAP and permutation importance mostly agree, and `addr_state` is the sharp exception.** Across all 4 folds and both models, `addr_state` ranks in the top 4–7 of 96 features by mean |SHAP| — comparable to `annual_inc` — while its permutation importance sits at essentially zero (even slightly negative for logistic_woe, meaning shuffling it did not measurably hurt PR-AUC in either direction). Two candidate explanations were checked and both ruled out rather than left unexamined: (1) small-sample overfitting in an early fold's fit cohort — ruled out because fold 2016 (93,153-row fit set — the post-calibration-holdout split from decision 9, smaller than Finding 3's raw embargoed-pool count for the same fold) shows one of the *worst* disagreements, not the best, so the pattern isn't monotonic in fit-set size; (2) correlation with a single other top-20 feature covering for it — ruled out because the nearest top-20 neighbour's correlation with `addr_state` is only r = 0.02–0.09 across the flagged fold/model cases, well under this project's own 0.3 flagging threshold (`nearest_correlated_neighbor`, computed on the WoE-encoded frame so it's comparable across both models). No confirmed explanation exists yet — recorded as an open item below rather than asserted, especially because `addr_state` is also where Phase 6's geographic fairness gap comes from, and "the model relies heavily on this per SHAP but it doesn't generalize per permutation" bears directly on how much weight that fairness finding should carry.

**The correlated-feature mechanism does explain some, but not most, of the other large disagreements.** Of the 8 fold/model cases with the single largest SHAP-vs-permutation rank gap, 4 had a plausible correlated top-20 partner (`revol_util`/`fico_range_low` r=0.38, `total_il_high_credit_limit`/`annual_inc` r=0.34, `inq_last_6mths`/`mths_since_recent_inq` r=0.40, `mo_sin_rcnt_tl`/`num_tl_op_past_12m` r=0.57); the other 4, including every `addr_state` case, were not — reported as "found" or "not found by this simple check," never asserted as the cause, since correlation alone can't rule out a higher-order interaction or plain permutation-importance sampling noise. A second real-data-only finding surfaced in the same table: `total_il_high_credit_limit` shows exactly zero SHAP and permutation importance in the 2014 and 2015 folds — confirmed directly against the raw data (not assumed) to be because the field is 100% null in both fit cohorts (18,065/18,065 and 39,786/39,786 missing), a Lending Club field-introduction artifact of the same kind Phase 1's drift analysis already documented for other columns, not a code bug.

**Independent confirmation of a Phase 3 finding, from an unrelated method.** The 6 features decision 7's WoE bin-collapse fix rescued (information value 0.0002–0.0011 at fit time) all rank in the bottom half of logistic_woe's 96 features by SHAP (27th–63rd); `acc_now_delinq` and `tax_liens` are within 1e-8 of exactly zero SHAP importance in the earliest fold because they are within 1e-8 of exactly zero *variance* there (`acc_now_delinq` is literally 0.0 for all 18,065 loans in the 2014 fit cohort) — checked against the raw column, not inferred from a rounded printout.

**A real formatting bug, caught by reading the actual output rather than trusting the code.** The first full run's local-explanation table printed `dti` as `30.170000076293945` instead of `30.17` — not a wrong number, but not something a credit adjudicator should ever see. Root cause: the raw modelling frame stores `dti` as float32, and Python's `float()` widening to float64 preserves the exact (imprecise) binary value rather than rounding it, even though numpy's own `str()` of the float32 value prints cleanly. Fixed with a small `_display_value` helper that rounds floats for display while leaving integers, strings, and categoricals untouched; regression-tested against the exact artifact (`test_display_value_rounds_float32_precision_artifacts`, which asserts the bug is reproducible before asserting the fix works).

---

## 14. Packaging: a model card that names the recommended model and says why, a light-touch README pass instead of a rewrite, a runner script whose own time estimate is stated as a guess, and a notebook that narrates rather than retrains

**Decision.** Four deliverables, each with a scoping decision worth stating rather than leaving implicit: `MODEL_CARD.md` (Mitchell et al. format, describing the logistic-WoE scorecard specifically — not "the model" generically, since Phase 7 gave three independent, already-measured reasons to prefer it over LightGBM: indistinguishable discrimination, no calibration repair needed, and interpretability by construction); `run_all.py`, a thin orchestrator that runs the existing ~20 `python -m module` commands in the README's own documented order, adding nothing new to what any one of them computes; `notebooks/walkthrough.ipynb`, executed for real (via `nbclient`/`nbconvert`, not hand-typed output) before being committed; and a light-touch pass over `README.md` (a model-card link, a `run_all.py` mention, a CI badge, the new fairness figure) rather than a structural rewrite into the brief's suggested problem→validation→results→limitations skeleton.

**Why light-touch, not a rewrite.** The brief's suggested order is "problem → why naive fails → validation design → results → three findings (one negative) → limitations." The README already does this in substance — findings table up front, the leakage headline immediately after, negative results kept in throughout (imbalance, SMOTE, the naive-threshold near-tie), limitations at the end — just not in that literal section order, because 14 findings accumulated incrementally across 8 phases rather than being planned as one document from the start. Reordering ~370 lines of cross-referenced findings risked introducing a real error (a broken internal link, a table separated from the prose that explains it) for a stylistic gain, against no functional one. Rewriting was the naive move here, not the careful one.

**Why `run_all.py`'s time estimate is stated as a guess for everything except Phase 7.** The full pipeline, start to finish, has never actually been run as one sequential command — each phase's script was run and verified on its own schedule over the life of this project, sometimes months apart, on different machines. Claiming a measured total would have been asserting something never checked. The one component with an actually-measured cost, `explainability.explain` at ~28 minutes, is labelled as such in the script and the estimate for everything else is labelled a guess, consistent with the "measure, don't assume" standard applied everywhere else in this project — including, here, to a claim about how long this project's own code takes to run.

**Why the notebook reads `reports/*.csv` instead of retraining.** The brief asks for "one notebook as a narrative walkthrough that imports from the modules," and a notebook that reran the full pipeline (an hour or more, per `run_all.py`'s estimate) every time someone opened it would be worse to actually use than one that reads the artifacts the modules already produced and committed. The notebook imports from `data.feature_audit`, `evaluation.decisioning`, and `explainability.explain` for two live, fast demonstrations of actual mechanisms (the exact threshold search, the exact linear-SHAP closed form) rather than only reading CSVs, so "imports from the modules" is true of the code, not just the import statements. Every cell was executed for real before being committed; one cell's markdown originally overclaimed ("the threshold lands strictly between the two blocks") until running it showed the true answer sits exactly at the lower block's shared score (`0.10 <= threshold < 0.30`, the same bound `tests/test_decisioning.py` already asserts) — caught by running the notebook, not by trusting what seemed like the obviously-true description of the mechanism.

**Two additions beyond the brief's four required items**: a GitHub Actions workflow running the 82-test suite on every push (closing a gap the prior repo audit flagged as "worth adding now that the repo is actually pushed"), and a fairness-gaps bar chart (`reports/figures/fairness_gaps.png`) — Phase 6 was the only phase with no committed figure until now.
...
- **Why `addr_state` gets high SHAP credit but near-zero permutation importance is unresolved.** Decision 13 ruled out small-sample overfitting and single-feature correlation as explanations; no alternative (a higher-order interaction, a systematic-but-small additive shift that a ranking metric on a 10,000-row subsample can't detect above its own sampling noise, or something else) has been confirmed. Worth resolving before leaning on this feature's SHAP importance in any fairness-remediation argument, since Phase 6 found this is also where the measured geographic disparity comes from.
- **The permutation-importance rank gap is a noisy statistic when the underlying importance value is near zero.** Several Phase 7 features (not just `addr_state`) have permutation importance close enough to zero, in both directions, that their exact rank among 96 features swings a lot fold to fold without the actual importance value changing much. `rank_table`'s `rank_gap` column is a useful flag for "look closer here," not a precise measurement of how much the two methods disagree.
- **`run_all.py` has never been timed end-to-end on the full raw dataset.** Its per-step estimates are stated guesses except for `explainability.explain` (measured at ~28 minutes). Worth timing a real full run once, on one machine, and updating the script's own estimates from a measurement rather than a guess.
- **The README restructure was light-touch, not the brief's literal suggested skeleton.** Decision 14 explains why; a reader skimming for exactly "problem → validation → results → limitations" in that section order won't find it in that order, even though every piece of content the brief asks for is present somewhere in the document.
